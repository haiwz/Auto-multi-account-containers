...
扩展现在会在 Firefox 内注册自定义协议 `ext+automac`，用来把外部链接交给某个受管容器打开。

- 在容器卡片的展开区域，可以看到两种链接模板：
- `稳定容器链接`：按 `profileId` 绑定，适合给启动器、脚本或后续 helper 使用
...
注意：这一步只完成 Firefox 内部的协议处理，不会把 `ext+automac://` 注册成 macOS / Windows / Linux 的系统级协议。仓库里的 Raycast 脚本之所以可用，是因为它会显式把链接交给 Firefox；如果你要在笔记应用里直接点击这类链接，仍然需要额外安装系统级 helper。
...
```text
open -a "Firefox" "ext+automac://open?slot=N#https://example.com"
```
...
The extension now registers a custom `ext+automac` protocol inside Firefox so external links can open in a managed container.

- The expanded profile card shows two link templates
- `Stable profile link` binds to `profileId`, which is better for launchers, scripts, and future helpers
...
This only enables protocol handling inside Firefox. It does not register `ext+automac://` as an OS-level scheme on macOS, Windows, or Linux. The bundled Raycast script works because it explicitly hands the link to Firefox; direct clicks from notes still require a separate system helper.
...
- the script explicitly opens Firefox with:
